...
if user_choice == "a":
    ...
    if user_choice == "c":
        ...
        if user_choice == "f":
            story = f"The shooter shoots and {RED}kills you instantly{RESET}. Jim manages to wheel unconscious Bethany through the back exit while the shooter is distracted."
            print(story)
    ...
    if user_choice == "d":
        ...
        G : The other hidden shopper panics, screams out your location to save his own skin, and the shooter executes you

        H : You all hold your breath, slip through the back loading-dock door, and escape together
        """
        print(story)
        ...
        if user_choice == "h":
            story = f"You, Jim, and Bethany escape together into the parking lot! However, a slow shopper gets caught in the crossfire and {RED}dies{RED}."
            print(story)
...
if user_choice == "b":
   ...
   S : Surrender
    ...
    if user_choice == "r":
        story = """
        You reach the pharmacy exit door, but it is jammed tight.
        What do you do?

        T : Smash the glass fire-alarm box next to the door to force the automatic locks open

        U : Hide in a nearby metal storage locker instead
        """
        print(story)
